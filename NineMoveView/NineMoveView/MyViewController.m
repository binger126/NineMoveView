...
static NSInteger margSpaceX = 10.0f;  //cell间距x
static NSInteger margSpaceY = 8.0f;  //cell间距y
static NSInteger columnNumber = 3.0f;// cell 列数
...
//    scrollView.bounces = YES;//控制控件遇到边框是否反弹
    ...
    scrollView.contentSize = CGSizeMake(self.view.frame.size.width*self.bigArray.count, self.view.frame.size.height);
    
    NSInteger spaceCount = columnNumber + 1 ; //(间隙count永远比列数多1)
    NSInteger width = (CGRectGetWidth(self.view.frame) - spaceCount*margSpaceX)/columnNumber;
    for (NSInteger i=0;i<self.bigArray.count;i++) {
        NSInteger x = (CGRectGetWidth(self.view.frame))*i;
...
-(void)longPressMethod:(UILongPressGestureRecognizer*)gesture{
    ...
    if (gesture.state==UIGestureRecognizerStateBegan) {
        ...
        NSInteger x = CGRectGetWidth(self.view.frame)*self.currentPage;
        fromPoint.x-=x;
...
//        for (UIButton *toBtn in self.frameArray[self.currentPage]) {
//            CGRect toFrame = toBtn.frame;
//            //根据偏移量更新frame
//            toFrame.origin.x-=x;
//            CGRect rect = CGRectIntersection(toFrame, self.btn.frame);
//            if (rect.size.width*rect.size.height>=self.btn.frame.size.width*self.btn.frame.size.height/2) {
//                return;
//            }
//            //判断拖动的按钮中心点是否在目标按钮区域范围内
//            if (CGRectContainsPoint(toFrame, self.btn.center)) {
...
//        }
        //向右切屏
        if (self.btn.center.x>CGRectGetWidth(self.view.frame)-10&&_lastPoint.x<self.btn.center.x) {
            if (self.currentPage==self.bigArray.count-1) {
                return;
            }
            lastArray = self.frameArray[self.currentPage];
            self.currentPage++;
            fromPoint.x+=(CGRectGetWidth(self.view.frame)*.8);
            self.btn.center = fromPoint;
            // 赋值给记录当前坐标的变
            [UIView animateWithDuration:.5 animations:^{
                self.scrollView.contentOffset = CGPointMake(self.currentPage*CGRectGetWidth(self.view.frame), 0);
            }];
        }
        //向左切屏
